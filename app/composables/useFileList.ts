...
export function useFileList(options?: { pageSize?: number, stateKey?: string, contentId?: string | null, fileType?: string | null }) {
  ...
  const mergeItems = (incoming: FileListItem[], replace = false) => {
    if (replace) {
  ...
  const fetchPage = async (opts?: { cursor?: string | null, replace?: boolean }) => {
    ...
    try {
      ...
      console.error('[file-list] failed to fetch', error)
      errorState.value = error instanceof Error ? error.message : 'Failed to load files'
      throw error
    } finally {
  ...
  const upsert = (entry: FileListItem) => {
    const normalized: FileListItem = {
      id: entry.id,
  ...
  const remove = (fileId: string) => {
    itemsState.value = itemsState.value.filter(item => item.id !== fileId)
  }
  ...
  const hasFile = (fileId: string | null | undefined) => {
    if (!fileId)
      return false
    return itemsState.value.some(item => item.id === fileId)
  }
